...
class BlogPreviewCard extends React.Component {
  render() {
    ...
    const imageURL = require('../../content/blog'+ data.fields.slug+'postcover.png');
       ...
       <CardContainer url={imageURL}>
        <BlackLayer>
          <BlogTitleSection>
            ...
            <BlogSubtitle>{data.frontmatter.subtitle}</BlogSubtitle>
            {/* <BlogAuthor>By: {data.author.name}</BlogAuthor> */}
            <BlogExcerpt>
              {data.frontmatter.description || data.excerpt}
            </BlogExcerpt>
            <DatePublished>{data.frontmatter.date}</DatePublished>
          </BlogTitleSection>
        </BlackLayer>
      </CardContainer>
    );
